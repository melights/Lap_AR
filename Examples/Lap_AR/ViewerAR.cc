...
void ViewerAR::Run()
{
    ...
    pangolin::Var<bool> menu_detectplane("menu.Insert AR",false,false);
    pangolin::Var<bool> menu_clear("menu.Clear All",false,false);
    pangolin::Var<bool> menu_drawim("menu.Draw Image",true,true);
    pangolin::Var<bool> menu_drawcube("menu.Draw AR",true,true);
    pangolin::Var<float> menu_cubesize("menu. AR Size",0.05,0.01,0.3);
...
/////////////// Load triangu
    ...
    const aiScene* scene = importer.ReadFile("./virus.ply", flags);
    ...
    for(uint v=0; v<mesh->mNumVertices; v++) {
        ...
        if(mesh->mColors[0]!=NULL) {
            colour.push_back({mesh->mColors[0][v].r,
                                       mesh->mColors[0][v].g,
                                       mesh->mColors[0][v].b});
        }
    }
    ...
    MakeTriangleStripIboForVbo(elementbuffer, elementbuffer.count_per_element, elementbuffer.num_elements);
...
void ViewerAR::DrawMesh(const float &size,const float x, const float y, const float z)
{
    pangolin::OpenGlMatrix M = pangolin::OpenGlMatrix::Translate(-x,-size-y,-z);
    glPushMatrix();
    M.Multiply();
